...
# !!!
def read_frames(video_path, list_of_fps, start_sec, end_sec=None, need_all_frames=True):
# def read_frames(video_path: Path, list_of_fps, start_sec, end_sec, need_all_frames: bool):
...
def reencode_with_audio(video_info: VideoInfo, output_path: Path, audio: torch.Tensor, sampling_rate: int):
...
# DOES NOT WORK
'''
def remux_with_audio(video_info, video_path, audio, output_path, sampling_rate):
    """
...
# !!!
    output_video_stream = output.add_stream('h264', rate=video_info.fps)
    ...
    for packet in video.demux(input_video_stream):
        if packet.dts is None: # skip the "flushing" packets that `demux` generates
            continue
        packet.stream = output_video_stream # assign the packet to the new stream
        output.mux(packet)
