...
class ModelConfig:
    ...
    def download_if_needed(self):
        ...
        download_model_if_needed(self.synch_path)

def model_cfg(name, model_dir):
    model_path = Path(os.path.join(model_dir, 'mmaudio_%s.pth' % name))
    vae_path = Path(os.path.join(model_dir, 'v1-%s.pth' % ('16' if '16k' in name else '44')))
    bigvgan_16k_path = Path(os.path.join(model_dir, 'best_netG.pt')) if '16k' in name else None
    synch_path = Path(os.path.join(model_dir, 'synchformer_state_dict.pth'))
    mode = '16k' if '16k' in name else '44k'
    return ModelConfig(name, model_path, vae_path, synch_path, bigvgan_16k_path, mode)
...
def generate(
    ...
    *,
    ...
    if clip_video is not None:
        ...
        clip_features = feature_utils.encode_video_with_clip(clip_video, batch_size=bs * clip_batch_size_multiplier)
        if image_input:
    ...
    if sync_video is not None and not image_input:
        ...
        sync_features = feature_utils.encode_video_with_sync(sync_video, batch_size=bs * sync_batch_size_multiplier)
    else:
    ...
    x0 = torch.randn(bs, net.latent_seq_len, net.latent_dim, device=device, dtype=dtype, generator=rng)
...
# !!!
    spec = feature_utils.decode(x1).to(dtype=dtype)
    # spec = feature_utils.decode(x1)
    audio = feature_utils.vocode(spec)
    return audio

# !!!
def load_video(video_path, duration_sec=None, load_all_frames=True) -> VideoInfo:
# def load_video(video_path: Path, duration_sec, load_all_frames=True) -> VideoInfo:
...
# !!!
    if duration_sec is None:
        duration_sec = float(len(all_frames) / orig_fps)
...
# !!!
def make_video(video_info, audio, in_vid, out_vid, out_aud, sampling_rate):

    # reencode_with_audio(video_info, out_vid, audio, sampling_rate)

    if os.path.abspath(in_vid) == os.path.abspath(out_vid):
        tmp_vid = os.path.join(os.path.dirname(in_vid), 'qq%s' % os.path.splitext(in_vid)[1])
        shutil.move(in_vid, tmp_vid)
        out_vid = in_vid
        in_vid = tmp_vid
    ffcmd = 'ffmpeg -v warning -y -i %s -i %s -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k "%s"' % (in_vid, out_aud, out_vid)
    os.system(ffcmd)
    os.remove(tmp_vid)
